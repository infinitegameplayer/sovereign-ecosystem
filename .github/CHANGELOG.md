...
- **Every citation in the repo now describes something real.** A full sweep found 41 non-resolving paths. 13 are patterns, 22 are destinations created at runtime or by you, and **6 were misleading and are fixed.** `Council Chamber/AI Interface/AI Interface Change Log.md` now ships as a starter log: four surfaces read it, the Meta-Governance Audit checks its rollover threshold and the Skills Governance Protocol requires skill changes to be logged in it, and it had never existed. Three Windows-only PowerShell tools cited by four protocol steps are no longer named, because they were never in the repo and no cross-platform template should assume a shell. The Taste Delta Report template, the Expression domain-template directory and the Archetypal Governance Map are now named as yours to create rather than as shipped surfaces.
