...
3. Run Pending Plan Reconciliation (proposal-only) and record outcomes.
4. Write breadcrumb updates into matched PendingPlan notes (activity/applicability/partial implementation/evidence/reconciliation notes) without changing status.
5. Run Reconciliation Sweep for inadvertent completions (quests, experiments, intake items).
