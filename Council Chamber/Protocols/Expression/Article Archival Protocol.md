...
version: 1.0
tier: foundational
purpose: Govern the completion phase of the article lifecycle - article archival, feedback loop and social breakdown archival
...
**Trigger:** Breakdown session complete - breakdown note exists with all posts drafted.
...
2. Move the article note to the Vault:
   ...
   - Filename does not change - preserves wikilink integrity from the breakdown note.
...
4. Distill observations - specific, not generic. Note whether each observation is new or recurring from a previous pass.
5. Apply a stability filter before updating the Codex:
   ...
   - **Pattern contradicted by next article:** Discard. Writing varies - single-article signals are noise, not signal.
6. Rolling Pattern Signals is a register of confirmed, stable patterns - not a snapshot of the last 3 articles. It changes slowly and intentionally.
...
3. Confirm `Scriptorium/Articles/` is clean - no orphaned breakdown notes or assets for this article.
...
- This protocol governs archival and feedback only - not drafting or breakdown creation.
- Cross-article feedback pass is not optional at Event 1 - it is the mechanism for continuity.
- Event 1 and Event 2 are separate sessions triggered by separate conditions; do not conflate them.
- The Rolling Pattern Signals section replaces previous content on each update - it is not a cumulative log.
-->
