...
- `Council Chamber/Codices/Humor Codex.md` ships as a seeded starter and is personalized during onboarding
...
- **External Model Routing.** The `Council Chamber/Codices/External Model Routing Codex.md` plus `Council Chamber/scripts/external-worker.mjs` give deterministic no-judgment work a verified path to secondary model providers. Voice-bound and judgment-bound work stays on the primary interface. The layer is only as active as the keys configured in `Council Chamber/scripts/.env`. Added v3.1.0.
...
- **Doctrine Lifecycle instruments.** `Council Chamber/scripts/doctrine-mass.mjs` and `Council Chamber/scripts/doctrine-references.mjs` plus `Council Chamber/Governance/Doctrine Mass Baseline.md` make doctrine growth visible: artifact and line counts per layer, inbound-link counts, orphan detection. Read-only by design. Plans have a visible lifecycle; these give the doctrine layer one too. Added v3.3.0.
- **Session coordination.** `Council Chamber/scripts/session-coordination.mjs` gives concurrent sessions against the same vault an advisory heads-up (register, check, close, cleanup). An advisory, not a lock. Added v3.3.0.
- **Backup encryption.** `Council Chamber/scripts/encrypt-stream.mjs` provides AES-256-GCM stream encryption for the moment a Sovereign adds an offsite backup destination. A door, not a default: the shipped backup script is local-to-local and needs no encryption until the vault leaves the machine. Added v3.3.0.
- **Guard hooks.** `pre-write-floor-guard.sh` (no edit to a trust-anchor file is ever silent) and `pre-tool-approval-gate.sh` (destructive-operation gate covering both the Bash and PowerShell tools) ship wired in `.claude/settings.json`. Added v3.3.0.
