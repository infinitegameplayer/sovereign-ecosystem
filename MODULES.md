...
- **Doctrine Lifecycle instruments.** `scripts/doctrine-mass.mjs` and `scripts/doctrine-references.mjs` plus `Council Chamber/Governance/Doctrine Mass Baseline.md` make doctrine growth visible: artifact and line counts per layer, inbound-link counts, orphan detection. Read-only by design. Plans have a visible lifecycle; these give the doctrine layer one too. Added v3.3.0.
- **Session coordination.** `scripts/session-coordination.mjs` gives concurrent sessions against the same vault an advisory heads-up (register, check, close, cleanup). An advisory, not a lock. Added v3.3.0.
- **Backup encryption.** `scripts/encrypt-stream.mjs` provides AES-256-GCM stream encryption for the moment a Sovereign adds an offsite backup destination. A door, not a default: the shipped backup script is local-to-local and needs no encryption until the vault leaves the machine. Added v3.3.0.
